...
public class MirrorDiscoveryTests
{
    ...
    {
        var svc = new MirrorDiscovery(BuildClient(HttpStatusCode.OK, "{}"));
    ...
    public async Task DiscoverMirrorAsync_JsonIndexResponse_ReturnsSuccess()
    {
        ...
        var svc = new MirrorDiscovery(BuildClient(HttpStatusCode.OK, indexJson));
    ...
    {
        var svc = new MirrorDiscovery(BuildClient(HttpStatusCode.NotFound, "not found"));
        var result = await svc.DiscoverMirrorAsync("https://mirror.example.com");
