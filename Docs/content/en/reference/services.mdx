...
For a new installation, call `DownloadAndLaunchInstanceAsync` on `IGameInstallationWorkflow` and inspect its result. Cancel using `CancelDownload(instanceId)`. See [Game lifecycle](/docs/architecture/game-lifecycle) for the distinction
