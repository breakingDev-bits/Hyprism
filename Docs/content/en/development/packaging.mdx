...
The macOS app bundle includes the generated `Packaging/macos/Hyprism.icns` icon derived from `logo.svg` and references it from `Info.plist`. The macOS publisher uses `plutil`, `hdiutil`, `codesign`, and `file`
