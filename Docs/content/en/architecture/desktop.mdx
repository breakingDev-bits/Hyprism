...
The shell uses Avalonia 12's [WindowDrawnDecorations](https://docs.avaloniaui.net/controls/primitives/windowdrawndecorations) model. The main window extends its client area into the decoration region. Windows selects `WindowDecorations="Full"` so native DWM state transitions and snap behavior remain available, while [WindowDecorations.axaml](../../../../Sources/HyPrism.Desktop/Styles/WindowDecorations.axaml) replaces Avalonia's default drawn decoration content

Other platforms keep `WindowDecorations="None"` through the platform-specific value and use the same custom shell. The [main window](../../../../Sources/HyPrism.Desktop/Shell/MainWindow.axaml) marks the title bar, caption buttons, and resize hit regions with `WindowDecorationProperties.ElementRole`; keep those roles on interactive chrome elements and do not reintroduce manual Win32 position updates
