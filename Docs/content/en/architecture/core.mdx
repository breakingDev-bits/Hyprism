...
`GameLaunchCoordinator` guards installed-instance launches. `GameInstallationWorkflow` chooses installation or update work and calls `GameLauncher` after preparation. The [launch workflow](/docs/architecture/game-lifecycle) describes their ordering and cancellation boundaries
...
See [Core contracts](/docs/reference/services) for integration examples and [Data and cache](/docs/architecture/data-and-cache) for persistence rules
