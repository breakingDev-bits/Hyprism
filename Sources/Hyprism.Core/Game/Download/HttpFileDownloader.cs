...
public class HttpFileDownloader : IFileDownloader
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="HttpFileDownloader"/> class.
    /// </summary>
        ...
        for (int attempt = 1; attempt <= MaxDownloadAttempts; attempt++)
        {
            ...
            try
            {
                ...
                else if (!response.IsSuccessStatusCode)
                {
                    ...
                    Logger.Error("Download", $"Download failed from {url}: HTTP {(int)response.StatusCode} {response.StatusCode}. Response: {errorBody[..Math.Min(500, errorBody.Length)]}");
                ...
                while ((bytesRead = await contentStream.ReadAsync(buffer, cancellationToken)) > 0)
                {
                    ...
                    // Preserve byte progress when the server omits Content-Length
                ...
                // Some servers end truncated responses without an HTTP error
