...
/// ID-based layout. Called once per session by <see cref="JsonProfileRepository"/>.
/// </summary>
    ...
    public static void MigrateUnresolvedFolders(string profilesDir, List<Profile> profiles, string appDir)
    {
        ...
        foreach (var folder in Directory.GetDirectories(profilesDir))
        {
            ...
            var matchedByName = profiles.FirstOrDefault(p =>
                string.Equals(LauncherUtilities.SanitizeFileName(p.Name ?? string.Empty), folderName, StringComparison.OrdinalIgnoreCase));

            if (matchedByName != null)
            {
                var target = LauncherUtilities.GetProfileFolderPath(appDir, matchedByName, createIfMissing: true, migrateLegacyByName: false);
                LauncherUtilities.TryMigrateSpecificProfileFolder(folder, target);
                continue;
            }
            ...
            {
                var target = LauncherUtilities.GetProfileFolderPath(appDir, matchedByMetadata, createIfMissing: true, migrateLegacyByName: false);
                LauncherUtilities.TryMigrateSpecificProfileFolder(folder, target);
                continue;
            }
